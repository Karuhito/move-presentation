...
def calc_erarnings(pre_cost):
    earnings = 2.87E+07 * math.log(pre_cost) - 4.44E+08
    return int(earnings)
...
earnings = calc_erarnings(pre_cost)
profit = calc_profit(earnings,cost)
profit_ratio = int(profit / cost * 100)

data_ad_cost = list(range(1000, 9001, 1))
data_earnings = [calc_erarnings(ad_cost*1.0E+04) for ad_cost in data_ad_cost]
data_profit = [calc_profit(earnings, ad_cost*1.0E+04 + fix_cost) for earnings, ad_cost in zip(data_earnings, data_ad_cost)]
...
ad_cost = st.sidebar.slider("広告費 (万円)", 1000, 9000)
fix_cost = 1000*1.0E+4
cost =  ad_cost + fix_cost

st.title("予想収益シミュレーション")
st.write(f"広告宣伝費:{int(ad_cost)}万円")
...
col3.metric("予想最大売上", f"{int(max_profit/1.0E+4)}万円")
